/* SarinhaPants01 — CHAR-11: a CALCA do Sarinha Mini Style (look Pop Musical).
   ...
   · + `legPaint: { thigh:'cloth', lowerLeg:'cloth' }` — o Assembler pinta a perna INTEIRA
...
(function () {
  ...
  var C = {
    ...
    yBarra: -0.100, rBarra: 0.252,   /* mais RETA que o short: e cos de calca */
    /* cos saliente no topo (R3: +0.008 de folga sobre a faixa) */
    cosY0: 0.018, cosY1: 0.042, cosSaliencia: 0.008,
    espessura: 0.012,
    ...
    bainhaAmp: 0.004,   /* calca quase reta na barra */
    sombraInterna: 0.28
  };

  var COR_PADRAO = 0x6E5A8F;   /* roxo suave (Pop Musical) */
  ...
  window.SARINHA_PERSONAGENS.createSarinhaPants01 = function (ctx, opts) {
    ...
    /* cos: anel saliente no topo, um tom mais escuro (le como costura) */
    var cT0 = anel(C.cosY0, C.rTopo + C.cosSaliencia, 0, false),
        cT1 = anel(C.cosY1, C.rTopo + C.cosSaliencia, 0, false);
    var corCos = new T.Color(cor).multiplyScalar(0.85).getHex();

    var corDentro = new T.Color(cor).lerp(new T.Color(0x6a5a8f), C.sombraInterna);
    var partes = [
      pinta(T, parede(T, cT1, cT0, false), corCos, 0),
      pinta(T, tampa(T, cT1, anel(C.cosY1, C.rTopo, 0, false), false), corCos, 0),
      pinta(T, tampa(T, anel(C.cosY0, C.rTopo, 0, false), cT0, false), corCos, 0),
    ...
    grupo.userData = {
      ...
      legPaint: { thigh: 'cloth', lowerLeg: 'cloth', foot: null }
    };
    return grupo;
  };

  window.SARINHA_PERSONAGENS.PANTS01_MEASURES = C;
})();
